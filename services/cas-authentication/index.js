...
function CASAuthentication(options) {
  ...
  if (options.service_url === undefined) {
    throw new Error('Hey!!! CAS Authentication requires a service_url parameter.');
  }
  ...
  this.cas_port = parsed_cas_url.protocol === 'http:' ? 80 : 8447;
  this.cas_path = parsed_cas_url.pathname;

  this.service_url = options.service_url;
console.log('this is service_url from inside cas index',this.service_url);
//  this.service_url = 'something something';
  ...
  this.session_info =
    ['2.0', '3.0', 'saml1.1'].indexOf(this.cas_version) >= 0 &&
